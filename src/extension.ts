...
const RELEASE_REPO = 'HalSysFin/Codex-Auth-Manager---Antigravity-Extension'
const RELEASE_CHECK_INTERVAL_MS = 12 * 60 * 60 * 1000
const RELEASE_CHECK_CACHE_KEY = 'authManager.releaseCheck'
const RELEASE_CHECK_USER_AGENT = 'cam-antigravity-extension'
...
class AuthManagerController {
  private readonly output = vscode.window.createOutputChannel('CAM Antigravity Extension')
        ...
        releaseCurrentLease: async () => {
          ...
          try {
            await this.client.releaseLease(this.state.leaseId, {
              machineId: this.state.machineId,
              agentId: this.state.agentId,
              reason: 'Manual fresh lease request from Antigravity extension',
            })
          } catch (error) {
  ...
  async releaseLease(): Promise<void> {
    ...
    try {
      const response = await this.client.releaseLease(this.state.leaseId, {
        machineId: this.state.machineId,
        agentId: this.state.agentId,
        reason: 'Released from Antigravity extension',
      })
      this.backendReachable = true
  ...
  async reloadCodexAuth(): Promise<void> {
    ...
    try {
      ...
      const selection = await vscode.window.showInformationMessage(
        'Codex auth file updated. Reload the Antigravity window if you want dependent tooling to reconnect immediately.',
        'Reload Window',
        'Later',
      )
    ...
    if (this.releaseLeaseOnShutdown() && this.state?.leaseId) {
      try {
        await this.client.releaseLease(this.state.leaseId, {
          machineId: this.state.machineId,
          agentId: this.state.agentId,
          reason: 'Released on Antigravity shutdown',
        })
